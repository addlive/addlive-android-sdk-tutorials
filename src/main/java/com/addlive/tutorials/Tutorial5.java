...
public class Tutorial5 extends Activity {
    ...
    class User {
        ...
        User(long userId, String videoSinkId) {
            this.userId = userId;
                ...
                new UIThreadResponder<Void>(this) {
                    ...
                    protected void handleResult(Void result) {
                        ...
                        for (User user : userMap.values()) {
                            ...
                            // Getting the resource with userId as id.
                            int resID = getResources().getIdentifier(
                                    String.valueOf(user.userId),
                                    "id", getPackageName());

                            // Getting the object with this id.
                            com.addlive.view.VideoView videoView =
                                    (com.addlive.view.VideoView) findViewById
                                            (resID);
            ...
            public void onVideoFrameSizeChanged(final VideoFrameSizeChangedEvent e) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        // Finding the user using the specific videoSinkId
                        String userId = null;
                        for (User user : userMap.values()) {
                        ...
                        if (userId != null) {
                            ...
                            // Getting the resource with userId as id.
                            int resID = getResources().getIdentifier(userId,
                                    "id", getPackageName());
                            ...
                            videoView.resolutionChanged(e.getWidth(),
                                    e.getHeight());
                        }
                    }
                });
            ...
            public void onMediaStreamEvent(final UserStateChangedEvent e) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        // Getting the record for the specific userId.
                        User user = userMap.get(e.getUserId());

                        // Updating the videoSinkId record.
                        user.videoSinkId = e.getVideoSinkId();
                        ...
                        if (e.isVideoPublished()) {
                            ...
                            videoView.stop();
                            videoView.setSinkId(e.getVideoSinkId());
                            videoView.start();
                        } else {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (e.isConnected()) {
                            ...
                            LinearLayout scroll =
                                    (LinearLayout) findViewById(R.id.scroll_linear);

                            com.addlive.view.VideoView videoView =
                                    new com.addlive.view.VideoView
                                            (Tutorial5.this);

                            videoView.setId((int) e.getUserId());
                            videoView.setLayoutParams(new LayoutParams(480, 640));

                            scroll.addView(videoView);
